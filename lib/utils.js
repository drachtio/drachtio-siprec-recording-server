...
const dtmfListenPort = config.has('rtpengine.localPort') ? config.get('rtpengine.localPort') : 22222;
const ngProtocol = config.has('rtpengine.ngProtocol') ? config.get('rtpengine.ngProtocol') : 'udp';
const {getRtpEngine, setRtpEngines} = require('@jambonz/rtpengine-utils')([], pino(), {
  //emitter: stats,
  dtmfListenPort,
  protocol: ngProtocol
});
