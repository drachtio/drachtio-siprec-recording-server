...
module.exports = async (req, res) => {
  ...
  if (!engine) {
    ...
    const tags = [
      "accepted:no",
      "sipStatus:480",
      `originator:${this.req.locals.originator}`,
    ];
  ...
  subscribeDTMF(logger, callid, from.params.tag, _onDTMF.bind(null, logger));
  dlg.on('destroy', onCallEnd.bind(null, del, callDetails, logger));
};
...
async function onCallEnd(del, opts, logger) {
  logger.info("call ended");
  del(opts)
  .then((response) => 
    logger.info(`response to rtpengine delete: ${JSON.stringify(response)}`))
