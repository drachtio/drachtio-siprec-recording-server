 ...
 *
 * no need to store this.calls.
 * no need to publish/subscribe in redis.
 */
...
const handler = (req, res) => {
  ...
  if (ctype.includes('multipart/mixed')) {
    ...
    logger.info(`received leg2 invite from freeswitch: ${req.source_address} with sessionID: ${req.get('X-Return-Token')}`);
