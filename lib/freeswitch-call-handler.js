  ...
  return payloadParser(opts)
    ...
    .then((opts) => {
      ...
      if (rapidsosheaderscount > 0) {
          ...
          // Alert-ID
          if (opts.recordingData['recording'].participant[0].extensiondata[0]['apkt:header'][0]['$']['label'] == 'X-RapidSOS-ID')
              console.log(opts.recordingData['recording'].participant[0].extensiondata[0]['apkt:header'][0]);
          else {
              console.log("Rejecting call gracefully. Missing required header.");
              res.send(503, 'Service Unavailable', { headers: {'X-RapidSOS-ID': 'Missing'}});
              return;
          }
          // Record-Call
          if (opts.recordingData['recording'].participant[0].extensiondata[0]['apkt:header'][1]['$']['label'] == 'X-RapidSOS-Record') {
              ...
              if (opts.recordingData['recording'].participant[0].extensiondata[0]['apkt:header'][1]['value'][0] == 'True') {
		  recording_filename = opts.recordingData['recording'].participant[0].extensiondata[0]['apkt:header'][2]['value'][0];
		  console.log("Forwarding call to Media Server for recording using filename: " + recording_filename);
              } else {
                  console.log("Rejecting call. No need to send to Media Server for recording.");
                  res.send(503, 'Not recording call', { headers: {'X-RapidSOS-Record': 'Not True'}});
                  return;
	      }		  
          } else {
              console.log("Rejecting call gracefully. Missing required header.");
              res.send(503, 'Service Unavailable', { headers: {'X-RapidSOS-Record': 'Missing'}});
              return;
          }
      }
	...
	'X-RapidSOS-Record-Filename': recording_filename
      };
    ...
    .catch((err) => {
      ...
      res.send(500, 'Internal Server Error', { headers: {'X-RapidSOS': 'Error'}});
      throw err;
    })
